...
public class TElementoAB<T> implements IElementoAB<T> {
    ...
    @Override
     ...
     public int cantidadHojas(){
         ...
         if(this.getHijoIzq()!=null){
             cantHojas=getHijoIzq().cantidadHojas();
         }
         if(this.getHijoDer()!=null){
             cantHojas=getHijoDer().cantidadHojas();
         }
         if(cantHojas==0){
             return 1;
         }else{
    ...
    public int nivel(Comparable unaEtiqueta){
        TElementoAB nodoActual = null;
        if(this.etiqueta.compareTo(unaEtiqueta) == 0)
            return 0;
        else if(this.etiqueta.compareTo(unaEtiqueta) > 0)
            nodoActual = this.hijoIzq;
        else
            nodoActual = this.hijoDer;
        
        if (nodoActual != null)
            return 1 + nodoActual.nivel(unaEtiqueta);
        return 0;
    }
