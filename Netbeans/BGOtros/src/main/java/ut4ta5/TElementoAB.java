...
// tiene los dos hijos, buscamos el lexicograficamente anterior
    ...
    @Override
    public int calcularCosto(int[] frecExito, int[] frecNoExito, int nivel, int[] indiceFE, int[] indiceFNE){
        int costo = 0;
        
        if(hijoIzq != null){
            costo = hijoIzq.calcularCosto(frecExito, frecNoExito, nivel+1, indiceFE, indiceFNE);
        }else{
        ...
        if(hijoDer != null){
            costo += hijoDer.calcularCosto(frecExito, frecNoExito, nivel+1, indiceFE, indiceFNE);
        }else{
            costo += frecNoExito[indiceFNE[0]++]*(nivel+1);
        }
