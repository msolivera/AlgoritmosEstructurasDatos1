...
// tiene los dos hijos, buscamos el lexicograficamente anterior
        ...
        @Override
        public int calcularCosto(int[] frecExito,int[] frecNoExito, int nivel, Integer indiceExito, Integer indiceNoExito)
        {
                int costoIzquierdo = 0;
                int costoDerecho = 0;
                if (hijoIzq != null) {
                        costoIzquierdo = hijoIzq.calcularCosto(frecExito, frecNoExito, nivel + 1, indiceExito, indiceNoExito);
                } else {
                        costoIzquierdo = frecNoExito[indiceNoExito] * (nivel + 1);
                        indiceNoExito++;
                }
                int costoActual = frecExito[indiceExito] * nivel;
                indiceExito++;

                if (hijoDer != null) {
                        costoDerecho = hijoDer.calcularCosto(frecExito, frecNoExito, nivel + 1, indiceExito, indiceNoExito);
                } else {
                        costoDerecho = frecNoExito[indiceNoExito] * (nivel + 1);
                        indiceNoExito++;
                }
                return costoActual + costoIzquierdo + costoDerecho;
        }
}
